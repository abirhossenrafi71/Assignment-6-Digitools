...
const Navbar = ({ navgationData, cartsData }) => {
    ...
    return (
        <nav className="container mx-auto w-full flex items-center  justify-between pt-8 px-4 lg:px-0    ">
            ...
            <button
                className="md:hidden text-3xl cursor-pointer m-0"
            ...
            {isOpen && (
                <div className="absolute top-20 right-0 w-full bg-white shadow-xl p-6 flex flex-col items-center gap-6 md:hidden z-50 rounded-b-2xl">
    ...
    return (
        <div className="flex  relative items-center gap-2 md:gap-3">
            ...
            {actions.map((btn, index) => (
                <button
                    key={index}
                    className={`px-4 md:px-6 py-2 rounded-full transition-all duration-300 cursor-pointer text-sm md:text-base ${index === 1
